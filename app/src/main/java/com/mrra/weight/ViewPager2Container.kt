...
class ViewPager2Container @JvmOverloads constructor(
    ...
    override fun onFinishInflate() {
        ...
        for (i in 0 until childCount) {
            val touchViewGroup = getChildAt(i)
            if (touchViewGroup is ViewPager2) {
                mViewPager2 = touchViewGroup
                break
            } else if (touchViewGroup is ViewPager2TouchListener) {
                for (j in 0 until touchViewGroup.childCount) {
                    val childView = touchViewGroup.getChildAt(j)
